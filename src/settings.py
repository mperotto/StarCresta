import pygame

# Configurações Gerais
LARGURA, ALTURA = 1920, 1080
FPS = 60
BASE_LARGURA, BASE_ALTURA = 480, 720
SCALE = ALTURA / BASE_ALTURA

# Gameplay
VELOCIDADE_JOGADOR_BASE = 280
VELOCIDADE_TIRO_BASE = -500
TEMPO_ENTRE_INIMIGOS = 1.0

def S(v):
    return int(round(v * SCALE))

# Escala velocidades
VELOCIDADE_JOGADOR = VELOCIDADE_JOGADOR_BASE * SCALE
VELOCIDADE_TIRO = VELOCIDADE_TIRO_BASE * SCALE

# Configurações de Jogo (Centralizadas)
VIDAS_INICIAIS = 3
MAX_TIROS_INIMIGOS = 18
BOSS_WAVE_INTERVAL = 55 # A cada quantas ondas o boss aparece
PONTOS_POR_ACOPLAMENTO = 1000
PONTOS_BONUS_BOSS = 500

